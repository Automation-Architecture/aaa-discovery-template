# Step 5 — Write the PRD via `/to-prd`

## Goal

A long-form Product Requirements Document that takes every locked decision from step 4 and turns them into:
...
`<YOUR_WORKSPACE>/client_projects/<initials>/repo/<project>/spec/prd.md` (v1.0)
...
1. Skim the output for:
   ...
   - Terminology consistent with the brief
   - Citations to the right module names from the brief's module breakdown
   - Open items match the actual remaining unknowns
2. Commit `spec/prd.md` to git locally (the repo is created in step 7 if it doesn't exist yet — for now just save the file)
3. Generate the DOCX if you'll be sending it for team feedback in step 8:
   ```bash
   pandoc spec/prd.md -o "<YOUR_CLIENT_DOCS_DIR>/<Client Full Name>/prd/<project-slug>/<Client>-<Project>-PRD-v1.0.docx" --from markdown --to docx
   ```
   Note: the `<YOUR_CLIENT_DOCS_DIR>` folder may not have a `prd/<project-slug>/` subfolder yet — `mkdir -p` it first. Path discipline: DOCX never lives in the repo.

## Don't do this

- **Don't paste financial info into the PRD.** Same global rule as the brief.
- **Don't have the PRD contradict the brief.** If the brief says "scope is X" and the PRD says "scope is Y", that's a sign you skipped the version-bump in step 9 or didn't carry over a decision. Reconcile.
...
PRD is written, committed to local git (or saved to disk if repo doesn't exist yet), DOCX is generated to `<YOUR_CLIENT_DOCS_DIR>/`, no financial info, no contradictions with the brief. Move to step 6.
