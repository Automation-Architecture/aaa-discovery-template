...
The second `/grill-me` round. Round 1 (step 4) locked the product. This round locks the build. By the end, every architecture-layer decision has a confirmed answer so the tech spec (step 11) is mostly transcription, not invention.
...
- The project's Slack sprint channel — look for the most recent PM "I'll assign this project to <@engineer>" message
- Any project management tool your org uses to track assignments

If no engineer is assigned, **stop**. Notify the operator (and PM if applicable). Don't run the architecture grill without them — operator-only architecture grills produce decisions engineers silently re-litigate mid-build.

### 2. Identify the project's engineering Slack channel

Convention: `#<slug>-sprint` (e.g. `#acme-support-agent-sprint`). Confirm via Slack search:
...
Append it to `spec/GRILL_SESSION.md` as a new section titled **"Round 2 — Architecture & Tech Stack (Engineer-Led)"**. If `spec/GRILL_SESSION.md` doesn't exist yet, scaffold it from `templates/GRILL_SESSION.md` (bundled in this skill) and fill in Round 1 first.
...
- **Don't let it drift back to product.** If the operator is making product changes during this round, that's a signal step 4 wasn't thorough enough — capture them, but don't let them dominate the architecture session.
