...
`<YOUR_CLIENT_DOCS_DIR>/<Client Full Business Name>/proposal/`
...
The proposal contains financial information (pricing, payment terms, invoicing schedule). **Do not surface any of this in the brief, PRD, tech spec, README, Jira, or any other tech artifact.** Tech docs stay technical. Financial content stays in `<YOUR_CLIENT_DOCS_DIR>/<Client>/proposal/` and the sales conversation only.
