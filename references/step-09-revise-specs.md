# Step 9 — Update specs with team feedback
...
- Any section the team flagged. Be honest about reversals — if the team said "drop the safety classifier," the docs should explicitly say "Per team feedback, no safety classifier in v1" rather than silently removing it.
- The deliverables sequence in the brief — mark step 8 ✅ done, step 9 ✅ done.
- A new "Resolved Items (post team feedback round)" section that lists what was decided
...
```bash
pandoc spec/project-brief.md -o "<YOUR_CLIENT_DOCS_DIR>/<Client>/prd/<slug>/<Client>-<Project>-Project-Brief-v1.1.docx" --from markdown --to docx
pandoc spec/prd.md -o "<YOUR_CLIENT_DOCS_DIR>/<Client>/prd/<slug>/<Client>-<Project>-PRD-v1.1.docx" --from markdown --to docx

# Delete the old versioned files so they don't accumulate
rm -f "<YOUR_CLIENT_DOCS_DIR>/<Client>/prd/<slug>/<Client>-<Project>-Project-Brief-v1.0.docx"
rm -f "<YOUR_CLIENT_DOCS_DIR>/<Client>/prd/<slug>/<Client>-<Project>-PRD-v1.0.docx"
```
...
- Discovery sequence in the brief shows steps 8 + 9 ✅ done

## Done when

Team feedback is visibly incorporated and the docs are at the new version. Move to step 10.
